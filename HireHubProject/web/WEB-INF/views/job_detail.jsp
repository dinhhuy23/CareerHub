...
<html lang="vi">
    ...
    <body class="app-page">
            ...
            <div class="container animate-fadeInUp">
                ...
                <div style="display: grid; grid-template-columns: minmax(0, 2fr) minmax(300px, 1fr); gap: var(--space-xl); align-items: start;">
                    ...
                    <div class="job-sidebar animate-fadeInUp" style="animation-delay: 0.2s;">
                        ...
                        <!-- Apply/Salary Box -->
                        <div class="glass-card" style="padding: var(--space-xl); margin-bottom: var(--space-xl); position: sticky; top: 100px;">
                            ...
                            <div style="text-align: center; margin-bottom: var(--space-xl); padding-bottom: var(--space-lg); border-bottom: 1px dashed var(--border-color);">
                                <div style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 8px; text-transform: uppercase; letter-spacing: 1px; font-weight: 600;">Thu nhập hấp dẫn</div>
                                <c:choose>
                                    <c:when test="${job.salaryMax != null}">
                                        <div class="text-success" style="font-size: 1.8rem; font-weight: 800; line-height: 1.2;">
                                            ...
                                            <div style="font-size: 1rem; color: var(--text-secondary); font-weight: 500;">${job.currencyCode} / tháng</div>
                                        </div>
                                    </c:when>
                                    <c:otherwise>
                            ...
                            <c:if test="${sessionScope.userRole == 'CANDIDATE'}">
                                ...
                                <button id="btnSaveJob" onclick="toggleSaveJob(${job.jobId})" class="btn ${isSaved ? 'btn-outline' : 'btn-primary'} btn-full" style="padding: 14px; font-weight: 600;">
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="${isSaved ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="2" style="margin-right: 8px; vertical-align: -4px;"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>
                                    <span id="saveJobText">${isSaved ? 'Đã lưu việc làm' : 'Lưu việc làm này'}</span>
                                </button>
                                <button onclick="openReportModal()" 
                                        class="btn btn-outline btn-full" 
                                        style="padding: 12px; font-weight: 600; color: #ff6b6b;">
                                    🚨 Báo cáo tin này
                                </button>
                            </c:if>
                            <c:if test="${empty sessionScope.userRole}">

                                <a href="${pageContext.request.contextPath}/login" class="btn btn-primary btn-full" style="padding: 14px; font-weight: 600;">Đăng nhập để ứng tuyển</a>
                            </c:if>
                            <c:if test="${sessionScope.userRole == 'RECRUITER' && sessionScope.userId == job.postedByRecruiterId}">
                                <!-- Note: The postedByRecruiterId comparison might not map straight to userId without DAO translation, but keeping it as a safe fallback -->
                                <a href="${pageContext.request.contextPath}/employer/jobs" class="btn btn-outline btn-full" style="padding: 14px;">Quản lý tin đăng</a>
                            </c:if>
                        </div>

                        <!-- Overview Box -->
                        <div class="glass-card" style="padding: var(--space-xl);">
                            <h3 style="font-size: 1.25rem; font-weight: 700; margin-bottom: var(--space-lg); border-bottom: 1px solid var(--border-color); padding-bottom: 12px;">Tổng quan</h3>
                            ...
                            <div style="display: flex; flex-direction: column; gap: 20px;">
                                ...
                                <div style="display: flex; align-items: flex-start; gap: 16px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-weight: 600; color: var(--text-primary); line-height: 1.4;">${job.locationName} <c:if test="${not empty job.addressDetail}">- ${job.addressDetail}</c:if></div>
                                        </div>
                                    </div>
        ...
        <!-- Apply Modal: Cửa sổ hiện lên khi nhấn "Ứng tuyển ngay" -->
        <div id="applyModal" class="modal-backdrop" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(4px);">
            <div class="glass-card animate-fadeInUp" style="width: 100%; max-width: 500px; padding: var(--space-2xl); position: relative;">
                ...
                <p style="color: var(--text-secondary); margin-bottom: var(--space-xl); font-size: 0.95rem;">Chào <strong>${sessionScope.userFullName}</strong>, hãy gửi thông tin để nhà tuyển dụng xem xét nhé.</p>

                <!-- Form gửi dữ liệu ứng tuyển tới ApplyController -->
                <form action="${pageContext.request.contextPath}/job/apply" method="POST" enctype="multipart/form-data">
                    <!-- Trường ẩn để gửi kèm ID công việc -->
                    <input type="hidden" name="jobId" value="${job.jobId}">

                    <div class="form-group" style="margin-bottom: var(--space-xl);">
                        <label style="font-weight: 600; margin-bottom: 8px; display: block;">Tải lên CV (PDF) <span style="color:var(--error)">*</span></label>
            ...
            <div class="glass-card" style="width:100%; max-width:500px; padding:20px; position:relative;">
                ...
                <form action="report" method="post">
                    ...
                    <div style="margin-bottom:15px;">
                        ...
                        <select name="reportTypeId" required style="width:100%; padding:8px;">
                            <c:forEach var="rt" items="${reportTypes}">
                                <option value="${rt.reportTypeId}">
                                    ${rt.reportName}
                                </option>
                            </c:forEach>
                        </select>
        ...
        <script>
            ...
            function openApplyModal() {
                ...
                document.body.style.overflow = 'hidden'; // Ngăn cuộn trang phía dưới
            }
