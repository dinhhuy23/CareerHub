...
//            ps.setString(7, user.getRoleName());
    ...
    public List<User> findAllCandidates() {
        ...
        // Lấy CV mới nhất (UpdatedAt DESC) từ UserCVs cho mỗi ứng viên
        // Danh sách ứng viên cũng sắp xếp theo UpdatedAt DESC (người dùng mới nhất lên đầu)
        String sql = "SELECT u.*, r.RoleCode, r.RoleName, "
                + "    (SELECT TOP 1 cv.UserCVId "
                + "     FROM UserCVs cv "
                + "     WHERE cv.UserId = u.UserId "
                + "       AND (cv.isDeleted = 0 OR cv.isDeleted IS NULL) "
                + "     ORDER BY cv.UpdatedAt DESC) AS LatestCvId "
                + "FROM Users u "
        ...
        try (Connection conn = dbContext.getConnection();
            ...
            while (rs.next()) {
                ...
                // Build cvUrl dạng /user/cv/view?id=X để dùng kết hợp contextPath ở JSP
                String latestCvId = rs.getString("LatestCvId");
                if (latestCvId != null) {
                    user.setCvUrl("/user/cv/view?id=" + latestCvId);
                }
                list.add(user);
            }
        } catch (SQLException e) {
